...
def read_data(filename):
    ...
    # fitness(NCTC8325-4), fitness(Newman), fitness(Dalbavancin); avoid Dalbavancin if possible
...
def train(params, features_array, bios_array, labels_array):
    ...
    for fold, (train_indices, val_indices) in enumerate(kf.split(features_array)):
        ...
        gen = predict_transformerv2.Predict_Staphylococcus_aureus_essential_ori_coding(params=params).to(device)
        ...
        for epoch in range(params['train_epochs_num']):
            ...
            if epoch % 10 == 0:
                print(
                    f"Epoch[{epoch}/{params['train_epochs_num']}] ****Test loss: {loss_test_one_epoch/len(test_loader):.6f}********test correlation_coefficient:{correlation_coefficient, spearman}"
                )

            metric.append(correlation_coefficient)

            # Save model if metric threshold met
            if correlation_coefficient > 0.85 and spearman > 0.6:
        ...
        if loss_kind == 'pearson':
            write_good_record(dict1=params, dict2=dict2, file_path='ori_coding_record_metric_pearson.txt')  # return  min(loss_test)

        elif loss_kind == 'pearson_mse':
            write_good_record(dict1=params, dict2=dict2, file_path='ori_coding_record_metric_pearson_mse.txt')  # return  min(loss_test)

        elif loss_kind == 'mse':
            write_good_record(dict1=params, dict2=dict2, file_path='ori_coding_record_metric_mse.txt')  # return  min(loss_test)
...
if __name__ == '__main__':
    ...
    # Data processing
    filename = '../data/Staphylococcus_aureus_essential_ori_coding.csv'
    
    df = pd.read_csv(filename, encoding="utf-8-sig")
    df = df.replace(r"^\s*$", pd.NA, regex=True).dropna(how="any").reset_index(drop=True)

    # Minimal conversions: sgRNA to uppercase; fitness to float (other columns kept as-is)
    if "guide_rna" in df.columns:
        df["guide_rna"] = df["guide_rna"].astype(str).str.upper()
    ...
    # Unpack common columns if present (else None)
    guides    = col_lists.get("guide_rna")
    fit18s    = col_lists.get("fitness")
    essentials = col_lists.get("essential")
    oris      = col_lists.get("ori")
    codings   = col_lists.get("coding")
    ...
    # Choose GPU
    params = {
        'device_num': 2, 'dropout_rate1': 0.3258494549467406, 'dropout_rate2': 0.2974783660130027,
        'dropout_rate_fc': 0.3134874750986153, 'embedding_dim1': 64, 'embedding_dim2': 256,
        'fc_hidden1': 109, 'fc_hidden2': 56, 'hidden_dim1': 1024, 'hidden_dim2': 256,
        'l2_regularization': 5e-05, 'latent_dim1': 64, 'latent_dim2': 256, 'num_head1': 8, 'num_head2': 8,
        'seq_len': 20, 'train_base_learning_rate': 0.0010350836441350173, 'train_batch_size': 512,
        'train_epochs_num': 500, 'transformer_num_layers1': 4, 'transformer_num_layers2': 8
    }
