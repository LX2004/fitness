...
def read_data(filename):
    ...
    number = 0
    for variant_guide, essential, fitness, gene in zip(
        df['variant_guide'], df['essential'], df['fitness'], df['gene']
    ):
        # cast to float
...
def make_dataset_sequences_bio(guides, fit18s, essentials):
    ...
    for sequence, score, essential in zip(guides, fit18s, essentials):
        ...
        feature = Dimer_split_seqs(sequence)  # use the full sequence as input
...
if __name__=='__main__':
    ...
    prediction_remove_bio = compute_scaler(
        model(torch.tensor(features_array).to(device))
    )

    model = torch.load('../models/bio_pearson_mse_predict_86_mertric=0.7014662700653662.pth').to(device)
    prediction = compute_scaler(
        model(
            torch.tensor(features_array).to(device),
            torch.tensor(biofeatures_array).to(device)
        )
    )
