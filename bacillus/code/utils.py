...
def write_good_record(dict1, dict2, file_path):
    ...
    # Check if file exists; create if not, otherwise append
...
def one_hot(sequence):
    bases = ['A', 'T', 'G', 'C']
    # Initialize an empty array to store the one-hot encoding
...
def trans_output_to_input(fake_im):
    # Encode generator outputs for the evaluator: first to raw sequences, then to a NumPy array
    sample_seq = []
...
def plot_test_prediction_result(output, label, epoch):
    ...
    plt.legend()
    plt.title('prediction value and ground truth value')
