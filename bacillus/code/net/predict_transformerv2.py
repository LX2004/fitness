...
class Predict_transformer_bacillus(torch.nn.Module):
    def __init__(self, params):
        ...
        self.final_fc1 = nn.Linear(
            params['latent_dim1'] + params['latent_dim2'] + 2,
            params['fc_hidden1']
    ...
    def forward(self, X, bio):
        ...
        # Enter convolutional module (if used)
        ...
        output = torch.cat((ori_pos, dim_pos, bio), dim=-1)  # fuse transformer outputs with biological metadata
...
class Predict_transformer_bacillus_remove_bio(torch.nn.Module):
    
    def __init__(self, params):
        super(Predict_transformer_bacillus_remove_bio, self).__init__()
        ...
        self.final_fc1 = nn.Linear(
            params['latent_dim1'] + params['latent_dim2'],
            params['fc_hidden1']
        )  # transformer-only features (no bio metadata)
        self.final_fc2 = nn.Linear(params['fc_hidden1'], params['fc_hidden2'])
        self.final_fc3 = nn.Linear(params['fc_hidden2'], 1)
        # self.bio_fc1 = nn.Linear(13, params['fc_hidden1'])
    ...
    def forward(self, X):
        ...
        output = torch.cat((ori_pos, dim_pos), dim=-1)  # fuse transformer outputs (no bio metadata)
        # output = self.mlp(ori_dim_pos)
...
class Predict_bacillus_essential_ori_coding(torch.nn.Module):
    def __init__(self, params):
        ...
        self.final_fc1 = nn.Linear(
            params['latent_dim1'] + params['latent_dim2'] + 6,
            params['fc_hidden1']
        )  # includes reserved slots for biological metadata
        self.final_fc2 = nn.Linear(params['fc_hidden1'], params['fc_hidden2'])
        self.final_fc3 = nn.Linear(params['fc_hidden2'], 1)
